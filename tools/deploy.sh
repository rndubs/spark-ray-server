...
cp systemd/spark-ray.service systemd/spark-dashboard.service ~/.config/systemd/user/
[ -f ~/.config/spark-orchestrator/capacity.toml ] || cp config/capacity.example.toml ~/.config/spark-orchestrator/capacity.toml
systemctl --user daemon-reload
systemctl --user enable spark-ray.service spark-dashboard.service >/dev/null 2>&1
...
echo "deployed; spark-ray + spark-dashboard active"
